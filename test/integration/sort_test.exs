defmodule Integration.SortTest do
  ...
  test "sorting in a pipeline", %{user: user} do
    send_message user, "@bot: operable:rules --list --for-command=rules| sort --field=rule"
    response = get_response
    assert length(response) == 3
    [first | remaining] = response
    IO.inspect first
    assert first["rule"] == "when command is operable:permissions with option[user] == /.*/ must have operable:manage_users"
    [second | [third]] = remaining
    assert second["rule"] == "when command is operable:permissions with option[role] == /.*/ must have operable:manage_roles"
    assert third["rule"] == "when command is operable:permissions with option[group] == /.*/ must have operable:manage_groups"
  end
